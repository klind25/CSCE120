...
int mapping (string loc) {
            ...
            else if (loc == "NewYork") num = 4;
            else if (loc == "Philadelphia") num = 3;
...
string rev_mapping (int num) {
            ...
            case 4: { loc = "NewYork"; break; }
            case 3: { loc = "Philadelphia"; break; } 
...
node::node (int arg_val, node *ptr = nullptr) : value(arg_val) {
    this->ptr = ptr;
}
...
node *node::getptr () {
    return this->ptr; // This return has to be updated
}
...
LinkedList::~LinkedList () {
    node* next;
    while (headptr) { // Until empty
        next = headptr->getptr(); // Temporary node
        delete headptr;
...
void LinkedList::push (int value) {
    if (headptr == nullptr) { // If empty
        node* newNode = new node(value, nullptr);
        newNode->setptr(headptr);
        headptr = newNode;
        return;
    }
    node* newNode = new node(value, headptr); // Not empty
    newNode->setptr(headptr); // Update where it points
    headptr = newNode;
...
int LinkedList::pop () {
    ...
    // TODO Here...
    node* newNode = headptr->getptr(); // Temp node
    value_to_remove = headptr->getvalue(); // Set return
...
void LinkedList::insertNode(int value, int reference_value) {
    ...
    if (reference_value == 0) { // Front
        newNode->setptr(headptr); // Update where it points
        headptr = newNode;
    }

    while(index->getvalue() != reference_value) { // Gets correct position
        index = index->getptr();
        if (index == nullptr) {
    ...
    newNode->setptr(index->getptr()); // Node points where index does
    index->setptr(newNode); // Index points to new node
...
int LinkedList::find_closest_less_than(int value) {
            ...
            while (newNode) { // Whole list
                if (newNode->getvalue() > value) {

                }
                else if (value - newNode->getvalue() < distance) { // Closest value less than
                    index = count;
                    distance = value - newNode->getvalue();
                }
                ++count; // Spot in list
                newNode = newNode->getptr(); // Next list item
            }

            if (index == 0) { // Head
                if (value < headptr->getvalue()) { // Before head
                    return 0;
                }
                else {
                    return headptr->getvalue(); // After head
                }
            }

            newNode = headptr;
            for (int i = 0; i < index; ++i) { // Get correct node
                newNode = newNode->getptr();
            }

            closest_less_than = newNode->getvalue(); // Obtain value
