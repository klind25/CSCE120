...
Image::Image(const Image& copyThis) : image(nullptr), width(0), height(0) { // Copy constructor
    *this = copyThis;

}

Image& Image::operator=(const Image& copyThis) { // Copy assignment
    if (this != &copyThis) {
        clear();
        width = copyThis.width; // Setting width and height
...
Image::~Image() { // Destructor gets called
    clear();
}
